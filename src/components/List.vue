    ...
    methods: {
      ...
      handleClick (val){
          ...
          if (search != "" && search != "取消"){
            ...
            $("#fixBox").show();
            $(".fix-item").append("<div class='fix-item-span'><span>"+search+"</span><i  class=\"mu-icon material-icons\">close</i></div>")
      ...
      openBottomSheet:function (type) {
        ...
        if(type == 1 && $(".fix-item div").length == 0) {
          ...
          $(".fix-item").append("<div class='fix-item-span' id='addItem"+type+"'><span>" + addContent + "</span><i  class=\"mu-icon material-icons\">close</i></div>")
          var num = $(".fix-item").children().length
          $("#nums").html(num)
        }else{
          var count=0;
